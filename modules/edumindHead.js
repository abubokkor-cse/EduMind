  ...
  */
  ...
  async showAvatar(avatar, onprogress = null) {
    ...
    if (this.isAvatarOnly) {
      ...
      // Only clear the avatar (gltf.scene parent), not the entire scene
      // This preserves lights, classroom, and other scene objects
      if (this.armature) {
        // Find the parent gltf.scene that contains the armature
        let avatarRoot = this.armature;
        while (avatarRoot.parent && avatarRoot.parent !== this.scene) {
          avatarRoot = avatarRoot.parent;
        }
        if (avatarRoot && avatarRoot.parent === this.scene) {
          this.scene.remove(avatarRoot);
          this.clearThree(avatarRoot);
        } else {
          // Fallback: just clear armature
          this.scene.remove(this.armature);
          this.clearThree(this.armature);
        }
      }
    }
    ...
    if (this.isAvatarOnly) {
      ...
      // Only add lights if they're not already in the scene
      if (this.lightAmbient && !this.lightAmbient.parent) {
        this.scene.add(this.lightAmbient);
      }
      if (this.lightDirect && !this.lightDirect.parent) {
        this.scene.add(this.lightDirect);
      }
      if (this.lightSpot && !this.lightSpot.parent) {
    ...
    if (r.audio !== undefined) {
      ...
      if (r.audio instanceof ArrayBuffer) {
        ...
        for (let i = 0; i < r.audio.length; i++) {
          let s = Math.max(-1, Math.min(1, r.audio[i]));
